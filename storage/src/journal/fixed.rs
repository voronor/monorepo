//! An append-only log for storing fixed length items on disk. In addition to replay, stored items
//! can be fetched directly by their `position` in the journal.
//!
...
impl<B: Blob, E: Storage<B>, const N: usize> Journal<B, E, N> {
    ...
    pub async fn init(runtime: E, cfg: Config) -> Result<Self, Error> {
        ...
        if !blobs.is_empty() {
            ...
            let mut previous_index = *it.next().unwrap().0;
            for (index, _) in it {
        ...
        // truncate the last blob if it's not the expected length, which might happen from unclean shutdown.
